...
begin
  ...
  try
    FPedidoRepository.consultaPedido(ADocCliente,oFDQuery);
    if (oFDQuery.IsEmpty) then
    begin
      result := nil;
      raise Exception.Create('O cliente com número de pedido '+ ADocCliente + ' não possui pedidos.');
    end;
    Result := TPedidoRetornoDTO.Create(
                                       TPizzaTamanhoEnum(GetEnumValue(TypeInfo(TPizzaTamanhoEnum), oFDQuery.FieldByName('de_tamanho').AsString)),
                                       TPizzaSaborEnum(GetEnumValue(TypeInfo(TPizzaSaborEnum), oFDQuery.FieldByName('de_sabor').AsString)),
